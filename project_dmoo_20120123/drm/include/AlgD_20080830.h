...
protected:
	...
	// prediction method
	enum PMETHOD {	PRANDI,		//!< randomly initialize population 
					PPOINT,		//!< predict several points
					PLINES,		//!< predict line segments
					PCENTR};	//!< predict the centre
	...
	CPopulationMO	mPop,	//!< current population
					mBest;	//!< best history population: mBest = mPop - mC
	...
	double	mSD;							//!< predicted standard deviation
public:
	//!\brief	constractor
	...
	//!\brief	whether change in this step
	//!\return	bool
	inline bool IsChange() {return mbChange;}
	...
	//!\brief	get the current and predicted centers
	//!\param	cc	current center
	//!\pram	pc	predicted center
	//|\return	bool
	bool Centers(std::vector<double>& cc, std::vector<double>& pc);
